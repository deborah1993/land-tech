    ...
    const getData = async () => {
      try {
        const response = await axios.get(
          `https://api.maptiler.com/data/93dc3483-bf44-40bb-9e97-12f4c21f98c4/features.json?key=${process.env.MAP_API_KEY}`
        );
    ...
    >
      ...
      >
        <Map
          ...
          mapStyle={`https://api.maptiler.com/maps/fc09eb56-b954-4808-8b36-f146ca7c5dfe/style.json?key=${process.env.MAP_API_KEY}`}
        >
          ...
          {data && (
            <Source id="my-data" type="geojson" data={data}>
              <Layer
                id="polygon"
                type="fill"
                source={`https://api.maptiler.com/maps/streets/style.json?key=${process.env.MAP_API_KEY}`}
                paint={{
                  "fill-color": "#888888",
                  "fill-opacity": 0.4,
                }}
